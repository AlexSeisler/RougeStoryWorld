  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      isScrolled 
        ...
        : activeLink === '/collection' ? 'bg-white/95 shadow-lg backdrop-blur-md' : 'bg-transparent'
    }`}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <Link 
            to="/"
            className="flex items-center space-x-2 md:space-x-4 magical-glow group relative"
            onClick={() => setActiveLink('/')}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
          >
            <div className="relative w-12 h-12 md:w-16 md:h-16 flex items-center justify-center">
              <div className={`absolute inset-0 bg-gradient-to-br from-purple-200 to-amber-200 rounded-full transition-all duration-300 ${
            ...
            <span className={`font-enchanted text-xl md:text-3xl font-bold transition-colors duration-300 ${
              isScrolled || activeLink === '/collection' ? 'text-gray-800' : 'text-white'
              ...
              <span className={`absolute -bottom-1 left-0 w-full h-0.5 bg-amber-500 transform origin-left transition-transform duration-300 ${
          ...
          <button 
            className={`md:hidden p-2 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-purple-500/50 ${
              isScrolled || activeLink === '/collection' ? 'hover:bg-gray-100' : 'hover:bg-white/10'
            }`}
            onClick={() => setIsMenuOpen(!isMenuOpen)}
            aria-label="Toggle menu"
          >
            {isMenuOpen ? (
              <X className={`w-6 h-6 ${isScrolled || activeLink === '/collection' ? 'text-gray-800' : 'text-white'}`} />
            ) : (
              <Menu className={`w-6 h-6 ${isScrolled || activeLink === '/collection' ? 'text-gray-800' : 'text-white'}`} />
            )}
          </button>
            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                {activeLink === link.path && (
                  <span className="absolute inset-x-0 -bottom-1 h-0.5 bg-amber-500 transform origin-left transition-transform duration-300"></span>
                )}
              </Component>
            ))}
          </div>
